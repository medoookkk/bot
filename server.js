...
● | ${prefix}tax لمعرفه الضريبه
● | ${prefix}stock لمعرفه المجود في البوت
...
** عشان تشتري فل داتا اكتب 1  buy sfa$**
...
لديك 3 دقائق قبل الالغاء.**`));
                ...
                message.channel.awaitMessages(filter, {
                  ...
                  delete cools [message.author.id + message.guild.id];
                        message.channel.send('** شكرا للشراء من  Master Of Gold\nNow Check Your DM**!')
                        message.author.send(`Your Accs :)\`\`\`json\n${Accs.join("\n")}\n\`\`\`سيتم حذف الرساله بعد 5 دقائق وبعد الحذف ينتهي الضمان  !`).then(M => M.delete(5 * 60 * 1000))
                    });
            }).catch(err => {
                  delete cools [message.author.id + message.guild.id];
                return message.channel.send('**:x: Please Open Your DM-افتح خاصك لو سمحت عاشن البوت يقدر يرسلك :x:**')
            })
        }
        if (cmd == 'Netflix') {
                    if (cools [message.author.id + message.guild.id] && cools [message.author.id + message.guild.id].status == "on")return message.reply("**لديك عملية شراء بل فعل.**"); 
            ...
            if (ahmed < 1) return message.channel.send("لا يوجد حسابات")
            if (ahmed < args) return message.channel.send("لا يوجد حسابات كافية")
            message.author.send('✅ Nothing.. Just Check If Your DM open or no-بشوفك فاتح الخاص ولا لا').then(() => {
...
var statuss = ["$help","$buy","$tax"]; // بالإمكان اضافة المزيد بدون التعديل في الكود
var secound = 10; // عدد الثواني المحددة
client.on("ready", () =>{
	var timeing = Math.floor(secound*1000);
	setInterval(function(){
		var ammount = statuss.length;
		var num = Math.floor(Math.random() * ammount);
		client.user.setActivity(statuss[num], {type: 'STREAMING', url: 'https://twitch.tv/ALPHA'})
	}, timeing)
});


const logid = "740490804040040518"; // ايدي روم اللوق
